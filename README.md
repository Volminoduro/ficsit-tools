...
- Après modification de `commun/` : `python3 scripts/langue.py` (ou `bash scripts/tout.sh`). Il assemble `commun/ficsit-commun.js` et `.css` (fichiers générés, à ne pas éditer : les sources sont `ficsit-lang.*`, `ficsit-paliers.*`, `langue.json`, `glossaire.json`) et place en tête de `<head>` de chaque page (marqueurs `FICSIT-LANG:START/END`) un lien vers eux, suffixé d'une empreinte de leur contenu (`?v=…`) pour contourner le cache après une mise à jour.
