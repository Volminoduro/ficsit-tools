...
- `node scripts/paliers_combinaisons.js satisfactory_infographie.html tout --ecrire` recalcule toutes les combinaisons précalculées de l'infographie et les écrit dans son payload (~10 min). Il lit les recettes, et le modèle de coût entre les marqueurs `MOTEUR:START` et `MOTEUR:END` du script de la page : à relancer après tout changement de palier ou de recette (donc après `payloads.py`).
