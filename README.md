...
- `donnees/emprises.json` : emprise au sol (largeur × longueur) des bâtiments de production et d'extraction, absente de la source, relevée à la main (champ `verifie` : recoupé ou non), plus le facteur de surcadençage (2,5). Sert au mode « Espace » de l'infographie : `payloads.py` en tire `em` (m² par machine), `ex` (m² par unité/min extraite, à 250 %) et `oc`.
